        ...
        while (($data = fgetcsv($handle)) !== FALSE) {
          ...
          for ($f = 0; $f < count($fieldNames); $f++) {
            switch ($fieldTypes[$f]) {
              ...
              case 'file':
                $logVariationFields .= "Importing file ".$fieldNames[$f]."(" . trim($data[$keyIndex[$fieldNames[$f]]]) . ") :: ";
                if (!empty($data[$keyIndex[$fieldNames[$f]]])) {
                  $imgIndex = trim($data[$keyIndex[$fieldNames[$f]]]);
                  //$files = glob('sites/default/files/' . $contentType . '/images/' . $imgIndex);
                  if ($fieldNames[$f] == 'field_newsletter_pdfs'){
                    $files = glob('sites/default/files/2019-02/'. $imgIndex);
                  }
                  if ($fieldNames[$f] == 'field_zip_file') {
                    $files = glob('sites/default/files/bloomberg/'. $imgIndex);
                  }
                  $fileExists = file_exists('sites/default/files/' . $imgIndex);
                  if (!$fileExists) {
                    $cfiles = [];
                    foreach ($files as $file_name) {
                      //$newfile = File::create(['uri' => 'public://' . $contentType . '/images/' . basename($file_name)]);
                      if ($fieldNames[$f] == 'field_newsletter_pdfs'){
                         $newfile = File::create(['uri' => 'public://2019-02/' . basename($file_name)]);
                      }
                      if ($fieldNames[$f] == 'field_zip_file') {
                        $newfile = File::create(['uri' => 'public://bloomberg/' . basename($file_name)]);
                      }
                      $newfile->save();
                      $cfiles[basename($file_name)] = $newfile;
                      $newfileId = $cfiles[basename($file_name)]->id();
                      $imageName = basename($file_name);
                    }
                    $nodeArray[$fieldNames[$f]] = [
                      [
                    ...
                    $logVariationFields .= "File uploaded successfully \n ";
                  }
                }
                $logVariationFields .= " Success \n";                
                break;
              ...
              default:
                ...
                $logVariationFields .= $fieldTypes[$f]." default \n";
                break;
            }
          }
